...
class Venta
{
    ...
    public function retornarTotalVentaNacional(){
        $motosVenta = $this->getColMotosV();
        $importeMotosNac = 0;
        foreach ($motosVenta as $motoV) {
            if ($motoV instanceof MotoNacional) {
                $importeMotosNac += $importeMotosNac + $motoV->darPrecioVenta();
            }
        }
        return $importeMotosNac;
    }

//    public function retornarTotalVentaNacional(Venta $unaVenta) {
//        $importeMotosNac = 0;
//        foreach ($unaVenta as $motoV) {
//            if ($motoV instanceof MotoNacional) {
//                $importeMotosNac += $importeMotosNac + $motoV->darPrecioVenta();
//            }
//        }
...
//    }
    public function informarVentasImportadas() {
        $motosVenta = $this->getColMotosV();
        $motosImportadasVenta = [];
        foreach ($motosVenta as $motoV) {
            if ($motoV instanceof MotoImportada) {
                $motosImportadasVenta[] = $motoV;
            }
        }
        return $motosImportadasVenta;
    }

//    public function informarVentasImportadas(Venta $unaVenta) {
//        $motosImportadasVenta = [];
//        foreach ($unaVenta as $motoV) {
//            if ($motoV instanceof MotoImportada) {
//                $motosImportadasVenta[] = $motoV;
